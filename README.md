...
<sub>上：未开启时，每个文件夹都在任务栏占一个按钮；下：开启后任务栏只剩常驻应用，文件夹全收进顶部悬浮栏。</sub>
...
- 悬浮栏的投影由**一个独立的、鼠标穿透的辅助窗口**绘制（`ShadowWindow`，带 `WS_EX_TRANSPARENT`）：
  这样既保留了阴影观感，悬浮栏自身的窗口命中区域又严格等于可见 UI —— 点阴影旁边不会挡住下层窗口。
